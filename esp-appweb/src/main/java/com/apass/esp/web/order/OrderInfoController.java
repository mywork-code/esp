...
//                return Response.fail("请选择所购买的商品");
    ...
    public Response requestRefund(Map<String, Object> paramMap) {
        try {
            ...
            String orderId = CommonUtils.getValue(paramMap, ORDER_ID);//订单Id
            ...
            orderService.requestRefund(requestId,orderId,userId, reason,memo);
     ...
     */
    @POST
    @Path("/changeRequestRefund")
    public Response changeRequestRefund(Map<String, Object> paramMap) {
        try {
        	String logStashSign = LogStashKey.ORDER_CHANGE_REQUEST_REFUND.getValue();
            String methodDesc = LogStashKey.ORDER_CHANGE_REQUEST_REFUND.getName();
            ...
            orderService.changeRequestRefund(requestId,orderId,userId, reason,memo);

        }catch (Exception e) {
            LOGGER.error("修改退款申请失败", e);
            return Response.fail(BusinessErrorCode.ORDER_CHANGE_REQUEST_REFUND);
        }
        return Response.success("修改退款申请成功");
    }
