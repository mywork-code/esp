...
`status` int NOT NULL COMMENT '退款状态（1:退款提交，等待商家审核；2：同意退款；3：取消退款；4：退款成功；5：退款失败）',
`status_d` datetime NOT NULL COMMENT '状态更新时间',
`reject_num` int DEFAULT 0  COMMENT '拒绝退款的次数',
`user_id` bigint NOT NULL COMMENT '用户id',
`main_order_id` varchar(255) DEFAULT '' COMMENT '主订单号',
...
`amt` decimal(15,4) DEFAULT  0 COMMENT '退款金额',
PRIMARY KEY (`id`)
...
PRIMARY KEY (`id`)
)ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='kv属性表';
