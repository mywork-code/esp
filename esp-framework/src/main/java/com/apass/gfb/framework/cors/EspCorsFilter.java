...
public final class EspCorsFilter implements Filter {

	private static final Log LOG = LogFactory.getLog(EspCorsFilter.class);
	...
	public void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse,
		...
		EspCorsFilter.CORSRequestType requestType = checkRequestType(request);

		// Adds CORS specific attributes to request.
		if (decorateRequest) {
			EspCorsFilter.decorateCORSProperties(request, requestType);
		}
		switch (requestType) {
		case SIMPLE:
	...
	protected void handleSimpleCORS(final HttpServletRequest request, final HttpServletResponse response,
		...
		EspCorsFilter.CORSRequestType requestType = checkRequestType(request);
		if (!(requestType == EspCorsFilter.CORSRequestType.SIMPLE || requestType == EspCorsFilter.CORSRequestType.ACTUAL)) {
			throw new IllegalArgumentException(sm.getString("corsFilter.wrongType2", EspCorsFilter.CORSRequestType.SIMPLE,
					EspCorsFilter.CORSRequestType.ACTUAL));
		}

		final String origin = request.getHeader(EspCorsFilter.REQUEST_HEADER_ORIGIN);
		...
		if (anyOriginAllowed && !supportsCredentials) {
			...
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, "*");
		} else {
			// If the resource supports credentials add a single
			// Access-Control-Allow-Origin header, with the value of the Origin
			// header as value.
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_ORIGIN, origin);
		}
		...
		if (supportsCredentials) {
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_CREDENTIALS, "true");
		}
		...
		if ((exposedHeaders != null) && (exposedHeaders.size() > 0)) {
			...
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_EXPOSE_HEADERS, exposedHeadersString);
		}
	...
	protected void handlePreflightCORS(final HttpServletRequest request, final HttpServletResponse response,
		...
		// Section 6.2.3
		String accessControlRequestMethod = request.getHeader(EspCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_METHOD);
		...
		String accessControlRequestHeadersHeader = request
				.getHeader(EspCorsFilter.REQUEST_HEADER_ACCESS_CONTROL_REQUEST_HEADERS);
		...
		if (preflightMaxAge > 0) {
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_MAX_AGE, String.valueOf(preflightMaxAge));
		}

		// Section 6.2.9
		response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_METHODS, accessControlRequestMethod);

		// Section 6.2.10
		if ((allowedHttpHeaders != null) && (!allowedHttpHeaders.isEmpty())) {
			response.addHeader(EspCorsFilter.RESPONSE_HEADER_ACCESS_CONTROL_ALLOW_HEADERS, join(allowedHttpHeaders, ","));
		}
	...
	private void handleInvalidCORS(final HttpServletRequest request, final HttpServletResponse response,
		...
		String origin = request.getHeader(EspCorsFilter.REQUEST_HEADER_ORIGIN);
	...
	protected static void decorateCORSProperties(final HttpServletRequest request,
		...
		case SIMPLE:
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST, Boolean.TRUE);
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
					request.getHeader(EspCorsFilter.REQUEST_HEADER_ORIGIN));
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_TYPE, corsRequestType.name().toLowerCase());
			break;
		case ACTUAL:
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST, Boolean.TRUE);
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_ORIGIN,
		...
		case PRE_FLIGHT:
			...
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_REQUEST_HEADERS, headers);
			break;
		case NOT_CORS:
			request.setAttribute(EspCorsFilter.HTTP_REQUEST_ATTRIBUTE_IS_CORS_REQUEST, Boolean.FALSE);
			break;
		default:
